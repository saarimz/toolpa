...
3. The manifest contract now supports `midi` / `midi-clip`, but the L2 builder schema still accepts MIDI requests even though the skeleton pipeline rejects them later.

Those were product-architecture issues, not cosmetic bugs. The remediation pass after this audit addressed the P1 items:
...
- `/api/build` now defaults to the LLM `ToolLoopAgent` create path.
...
| L1 tools | 10 |
| L2 builders | 6 |
| Installed manifests | 14 |
| Generated manifests visible at runtime | 2 |
| Instrument types | builder 6, sample 5, synth 3, effect 1, midi 1 |
| Document types | files 6, pattern 4, synth-scene 3, audio-stream 2, midi-clip 1 |
...
`/api/build` now calls `runBuilderToolLoopAgent()` by default. `/api/build/edit` calls `runBuilderEditAgent()`, which also uses `ToolLoopAgent`. `runBuilderAgent()` remains useful as the deterministic skeleton/scaffold helper and for tests, but it is no longer the default user-facing create path.
...
Pattern, SynthScene, MidiClip, and audio-stream patch contracts keep tool output inspectable. This is the right foundation for an AI-native DAW because generated output is not trapped inside transient Web Audio nodes.
...
The dashboard can expose an unregistered generated tool while the generated audit reports clean. That bypasses the intended registry/snapshot source of truth and weakens the audit as a safety gate.
...
- `handleBuildRequest()` defaults to `runBuilderAgent`.
- `runBuilderAgent()` reads schema/reference files, instantiates the canonical skeleton, validates, and registers.
- The LLM-backed `runBuilderToolLoopAgent()` exists but is not the default create path.
...
- API build test asserting which runner `/api/build` uses by default.
- UI copy test so the visible builder text matches the real behavior.

Fix applied:

- `/api/build` defaults to `runBuilderToolLoopAgent()` instead of `runBuilderAgent()`.
- The handler dependency type now matches the tool-loop runner.
- Existing streaming behavior remains covered through handler tests.
...
- `BuildToolRequestSchema` allows every non-builder instrument type, including `midi`.
- `runBuilderAgent()` rejects unsupported instrument types after request validation: current skeletons cover sample, synth, effect, and hybrid only.
- There is no MIDI builder profile or MIDI skeleton.

Impact:

The API contract accepts MIDI builder requests that the builder cannot fulfill. This creates a bad failure mode for the newest product surface.
...
### P2: Documentation Is Behind The Actual Product Inventory

Evidence:

- README lists installed L1 tools but omits `sample-analysis`, `midi-generator`, generated effect, and generated sine synth state.
- README still mentions generated examples such as `microtonal-sampler` and `vocal-stutter`, while current generated registry contains `sine-wave-synth` and the runtime also sees `harmonic-distrotion-effect`.

Impact:

The docs no longer match the dashboard. That makes audits, onboarding, and future L2 builder work more error-prone.

Recommendation:

- Update README after resolving the generated registry blind spot.
...
The static audit checks required files, client boundary, placeholder text, disallowed imports, browser storage globals, exfiltration globals, dynamic import shape, dynamic code, and module-level audio construction.

Gaps:

- It only applies to generated tools during registration.
- It does not catch generated tools that enter the runtime outside `.audit/generated-tools.json`.
...
- Extend generated audit with registry-vs-in-tree reconciliation.
...
1. Reconcile generated registry and in-tree generated manifests.
2. Decide whether unregistered generated tools are hidden, marked unhealthy, or auto-registered only after gates.
3. Update generated audit tests to cover missing registry entries.
4. Update README inventory from current manifests.
...
4. Align `BuildToolRequestSchema.instrumentType` with supported skeletons.

### Phase 3: Extend MIDI Platform Support

1. Add a MIDI builder profile or reject MIDI builder requests early.
...
- `pnpm test:unit`: passed 126 files / 446 tests. Vitest printed happy-dom abort teardown stacks, but exited 0.
...
- Platform hardening: `ready`, `10 L1 tools and 6 L2 builders pass platform hardening gates.`
...
The app architecture is promising and already has unusually good contract and audio verification infrastructure. The next architectural move should be to close the source-of-truth gap between runtime manifests and generated registry metadata, then make the L2 builder behavior match the product promise. After that, MIDI builder support and AI timeout/fallback parity are the highest-leverage improvements.
